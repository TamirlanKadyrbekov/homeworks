...
func main() {
	...
	people := PersonSlice{NewPerson("Tamirlan", "Kadyrbekov"), NewPerson("Aidana", "Kemoriva"), NewPerson("Bekaris", "Kuanish")}
	fmt.Printf("Before: %v\n", people)
	sort.Sort(people)
	fmt.Printf("After: %v\n", people)
	...
	fmt.Println(IsPalindrome(them))

	fmt.Println("Problem 3")
	add := func(x, y int) int { return x + y }
	fmt.Println(Fold([]int{1, 2, 3, 4, 5}, 0, add))
	mult := func(x, y int) int { return x * y }
	fmt.Println(Fold([]int{1, 2, 3, 4, 5}, 1, mult))
}
